...
int main(int argc, char** argv)
{
    ...
    if (!strcmp(argv[1],"-h") || !strcmp(argv[1], "--help") || !strcmp(argv[2],"-h") || !strcmp(argv[2], "--help"))
    {
		...
		cout << "	-b or --bin - switch the trash bin icon (you may add -empty or -full if you only want to change one trash bin icon like --bin-full or --bin-empty)\n";
		cout << "	-h or --home - switch the home directory icon\n";
	...
	if (!file.is_open())
	{
        std::cout << "File not found or not correct priviledge - Try again with sudo\n";
        return -1;
    }
	...
	else if (!strcmp(argv[2], "-h") || !strcmp(argv[2], "--home"))
	{
		if (OS == 0)
		{
